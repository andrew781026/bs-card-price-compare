...
const fileFolder = path.join(__dirname, process.env.TASKS_FOLDER);
const filePath = path.join(fileFolder, 'tasks.txt');
...
const extractAndVerifyToken = async (headers) => {
  ...
  const response = await axios.get(`http://${authAddress}/verify-token/` + token);
...
app.post('/tasks', async (req, res) => {
  try {
    ...
    fs.appendFile(filePath, jsonTask + 'TASK_SPLIT\n', (err) => {
      if (err) {
        console.log(err);
        return res.status(500).json({message: 'Storing the task failed.', filePath});
      }
